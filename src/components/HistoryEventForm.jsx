        ...
        <div className="animate-[fadeIn_0.3s_ease] font-sans pb-10">
            ...
            {showCreateModal && (
                <div className="fixed inset-0 z-[1050] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 font-sans">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden relative animate-[settingsSlideIn_0.3s_ease-out] flex flex-col max-h-[90vh]">
                        <form onSubmit={handleSubmit} className="flex flex-col h-full min-h-0">
                            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
                                <h3 className="font-bold text-slate-800 text-lg m-0 font-heading tracking-wide">Tambah Event Baru</h3>
                                <button type="button" className="text-slate-400 hover:text-red-500 transition-colors text-xl" onClick={() => setShowCreateModal(false)}>
                ...
                <div className="flex justify-start mt-2">
                    <button type="button" onClick={addSession} className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2.5 rounded-xl font-bold transition-all shadow-sm active:scale-95 flex items-center gap-2 text-sm">
                        <i className="fas fa-plus"></i> Tambah Shift Lain
                    </button>
                </div>
                            </div>
                            <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-end gap-3 shrink-0">
                                <button type="button" className="px-5 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold rounded-xl transition-colors text-sm" onClick={() => setShowCreateModal(false)} disabled={isSubmitting}>Batal</button>
                                <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 bg-maroon-primary hover:bg-maroon-dark text-white font-bold rounded-xl transition-all shadow-md active:scale-95 disabled:opacity-50 text-sm flex items-center justify-center gap-2">
                                    {isSubmitting ? <><div className="modern-spinner w-4 h-4 border-2 border-t-white"></div> Menyimpan...</> : 'Simpan Event & Shift'}
                                </button>
                            </div>
                        </form>
                    </div>
